...
def apply_ain(exp, dio, plot_uniq_inx):
    ...
    train_rank_matrix = gen_rank_matrix(exp.data_to_run.train_ia_perf_matrix)
...
#         if "PROTEUS" not in runtime_file_path:
#             continue
        ...
        for fold_inx in range(num_folds): #for each fold
            ...
            train_ia_perf_matrix, train_i_ft_matrix, test_ia_perf_matrix, test_i_ft_matrix, train_inst_inx_arr, test_inst_inx_arr = dio.extract_fold_data(fold_inx)
            ...
            if fold_inx == num_folds-1:
                num_solved_ss, par10_ss, avg_rank_ss = dio.evaluate_pred_matrix(result_ia_perf_pred_matrix)
