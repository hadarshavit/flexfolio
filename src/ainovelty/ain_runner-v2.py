...
from src.data_io import gen_issolved_matrix, write_overall_perf_summary,\
    calc_total_ft_cost
from src.feature_analyzer import ft_selection_post_process


'''
    NOTES:
        -- ft importances in rf are normalized
        -- ft importance with RF regression (+classification): http://blog.datadive.net/selecting-good-features-part-iii-random-forests/
...
#     gen_aa_superior_matrix(train_rank_matrix)
    ...
    print "# instance features are selected: ", len(top_ft_inx_arr), " out of ", dio.num_features
    
    
    
    ft_selection_post_process(dio.i_ft_cost_matrix, dio.i_ft_step_membership_matrix, top_ft_inx_arr, top_ft_importance_arr)
    ...
    if exp.alg_subset_selection:
        ...
        ## ignore for now- only use optimal portfolio
...
#     test_ia_rank_matrix = dio.ia_rank_matrix[exp.data_to_run.test_inst_inx_arr, sorted_alg_per_cls]
    ...
    num_solved, par1, par10, avg_rank = evaluate_pred_matrix(test_ia_perf_matrix, test_ia_issolved_matrix, test_ia_rank_matrix, fold_pred_matrix, exp.data_to_run.test_unsolved_inst_list)
    perc_solved = (num_solved / float(len(test_ia_perf_matrix)))
    
    print("FOLD #", (plot_uniq_inx+1)," - # num_solved: ", num_solved, "par1_ss", par1, "par10_ss: ", par10, "avg_rank_ss: ", avg_rank)
    logger.info("FOLD #%d - # solved instances: %d out of %d (%.3f), Par1: %.2f, Par10: %.2f, Avg Rank: %.2f" %((plot_uniq_inx+1), num_solved, len(test_ia_perf_matrix), perc_solved, par1, par10, avg_rank))
...
def apply_ain_cv(bench_root_folder, exp):
    '''
    ...
    ## performance data including # solved instaces, par10, avg rank
    ##overall_perf_arr = np.zeros((len(bench_folder_list), 3))
    overall_perf_list = []
    ##data_inx = 0
...
#         if "CSP" in runtime_file_path or "PREMARSHALLING" in runtime_file_path or "QBF" in runtime_file_path:
            continue
        ...
        for fold_inx in range(num_folds): #for each fold
                ...
                if dio.num_algs > len(sorted_alg_per_cls):
                    ...
                    for alg_inx in range(dio.num_algs):
                        if alg_inx in sorted_alg_per_cls:
                            ...
                            test_alg_arr_inx += 1 ##SAT12-RAND fix
                        else:
            ...
            logger.debug("Finished processing fold #%d" %(fold_inx+1))

            if fold_inx == num_folds-1:
                num_solved_ss, par1_ss, par10_ss, avg_rank_ss = dio.evaluate_pred_matrix(result_ia_perf_pred_matrix, dio.unsolved_inst_list)
                perc_solved = (num_solved_ss / float(dio.num_insts))
                print("num_solved_ss: ", num_solved_ss, "par1_ss: ", par1_ss, "par10_ss: ", par10_ss, "avg_rank_ss: ", avg_rank_ss)
                logger.info("# solved instances: %d out of %d (%.3f), Par1: %.2f, Par10: %.2f, Avg Rank: %.2f" %(num_solved_ss, dio.num_insts, perc_solved, par1_ss, par10_ss, avg_rank_ss))
                
#                 overall_perf_arr[data_inx][0] = num_solved_ss
#                 overall_perf_arr[data_inx][1] = par10_ss
#                 overall_perf_arr[data_inx][2] = avg_rank_ss
#                 data_inx += 1
                overall_perf_list.append([dio.dataset_name, 
                                          num_solved_ss, 
                                          (num_solved_ss/float(dio.num_insts)), 
                                          ((dio.perf.num_inst_solved-num_solved_ss)/float(dio.num_insts)), 
                                          par1_ss, par10_ss, avg_rank_ss])
    ...
    ## write overall performance summary to a csv file
    out_perf_summary_file = os.path.join(__output_folder__, exp.output_folder_name+"-perf-summary.csv")
    write_overall_perf_summary(out_perf_summary_file, overall_perf_list)
...
def main():
    ...
    apply_ain_cv(bench_root_folder, exp)
