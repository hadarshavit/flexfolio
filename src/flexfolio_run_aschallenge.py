    ...
    def normal_mode(self, aslib_data, ex_dic, se_dic, al_dic, pwd, output_fn):
        ...
        with open(output_fn, "w") as fp:
            ...
            for inst_ in aslib_data.itervalues():
                ...
                #predict
                dic_thread_schedule = SelectionBase.select(selector_name, se_dic, features, pwd)
                selected_solver = dic_thread_schedule[1][0][0]
                ...
                schedule_length = len(dic_thread_schedule[1])
                for id_, (algo_name, (_, time)) in enumerate(dic_thread_schedule[1]):
                    if id_ == schedule_length - 1:
                        time = sys.maxint
                    fp.write("%s, %d, %s, %d\n" %(inst_._name, id_+1+n_presolvers, algo_name, time))
                fp.flush()
