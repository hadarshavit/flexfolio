...
class CosealReader(object):
    ...
    def parse_coseal(self, coseal_dir, args_):
        ...
        if not self.remove_features(): # empty feature set
            self.metainfo.options.feature_steps = None # deleting specified feature groups
    ...
    def check_instances(self):
        ...
        Printer.print_c("Algorithms: \t\t %d" %(len(self.metainfo.algorithms)))
        Printer.print_c("Incomplete Feature Vector: \t %d (maybe imputed)" %(n_no_feats))
        ...
        Printer.print_c("Average Feature Costs: \t %.4f" %(feature_costs / n_instances))
    ...
    def remove_features(self):
        ...
        feature_group_dict = self.metainfo.feature_group_dict
        features = self.metainfo.options.features
