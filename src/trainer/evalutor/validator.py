...
class Stats(object):
    ...
    def print_stats(self, oracle_avg_time, oracle_spend_time_dict, oracle_par10, oracle_tos, cutoff):
        ...
        for thread, time in self.thread_time_dic.items():
            ...
            self.thread_avg_dic[thread] = self._extract_par1_from_par10(time, self.thread_timeout_dic[thread],
                                                                        cutoff) / self._test_n
        ...
        Printer.print_c("PAR10 per #Thread: %s" % (str(self.thread_par10_dic)))
        ...
        Printer.print_c("Timeouts: %s" % (str(thread_timeout_dic_wo)))
        solved_wo_unsolveable = dict(
            [thread, (solved_y * self._test_n) / (self._test_n - self.unsolved)] for thread, solved_y in
            solved.iteritems())
        Printer.print_c("Solved (perc): %s" % (str(solved_wo_unsolveable)))
        par1_wo_unsolveable = dict(
            [thread, ((avg * self._test_n) - (cutoff * self.unsolved)) / (self._test_n - self.unsolved)] for thread, avg
            in self.thread_avg_dic.iteritems())
        Printer.print_c("PAR1: %s" % (str(par1_wo_unsolveable)))
        par10_wo_unsolveable = dict(
            [thread, ((par10 * self._test_n) - (10 * cutoff * self.unsolved)) / (self._test_n - self.unsolved)] for
            thread, par10 in self.thread_par10_dic.iteritems())
...
class Validator(object):
            ...
            for conf_name, conf_dict in configs_dict.items():
                if conf_dict.get("presolving_time"):
                    for core, pre_solve_time in conf_dict.get("presolving_time").iteritems():
                        ...
                        solver_schedule[core] = solver_schedule.get(core,
                                                                    {"claspfolio": meta_info.algorithm_cutoff_time})
        ...
        for instance in instance_test.values():
            ...
            if instance._pre_solved and using_features:  #SBS and aspeed do not presolve anything
                ...
                if not instance._features is None and feature_time < meta_info.options.feat_time:  #TODO: does not make sense in case of feature imputation
                    dic_thread_schedule = self._select(meta_info.options.approach, selection_dic,
                                                    copy.deepcopy(instance._features))
                else:
            ...
            stats._test_n += 1
            stats.spend_time = dict(
                (solver, stats.spend_time.get(solver, 0) + spend_time_dict.get(solver, 0)) for solver in solver_list)
            ...
            for t in thread_time_dic.keys():
                ...
                # selection_dic = updater.update_models(thread_time_dic[1], instance.get_features(), instance.get_class(), dic_thread_schedule[0][0], instance)
                #-------------------------------------- up_dic["n_new"] += 1
        # ------------- Printer.print_verbose(str(global_thread_timeout_dic))
        Printer.print_c(
            "PAR10 on current test set: %.2f (%d instances)" % (par10_per_set / len(instance_test), len(instance_test)))
    ...
    def evaluate_invalids(self, time_invalid_dic, backup_solver, cutoff):
        ...
        Printer.print_c(
            ">>>>> Stats of instances with invalid feature list (no features, too few features, unknown features):")
    ...
    def _get_time(self, dic_thread_schedule, times, ftime, solver_list, cutoff, solver_schedule, satzilla_mode=False):
        '''
            look for runtime of given runtimes (times) while selecting solvers from list_conf_scores
            Parameter:
                list_conf_scores: sorted list with (solver_name,(score, schedule_time)
        ...
        for threads in range(1, max_threads + 1):
            ...
            if satzilla_mode:  # first step: schedule
                ...
                if threads == 1:
                    ...
                    selected = ["claspfolio", dic_thread_schedule[threads][0][0]]
                else:
            ...
            if not thread_timeout_dic.get(threads) == 0:
                ...
                if used_time < ftime:  # selected solver can only run if the feature computation was done
                    used_time = ftime
